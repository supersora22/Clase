...
DELIMITER $$
CREATE OR REPLACE TRIGGER triggerEstadoPrestamo
...
DELIMITER $$
CREATE OR REPLACE TRIGGER triggerContadorPrestamos
AFTER INSERT ON Prestamo
FOR EACH ROW
BEGIN
    UPDATE Usuarios
...
-- 3.4. Transacciones (Procedimientos)
DELIMITER $$
CREATE OR REPLACE PROCEDURE DevolverYPerdonar(IN p_id_prestamo INT, IN p_id_usuario INT)
BEGIN
    DECLARE v_existe INT DEFAULT 0;
    
    SELECT COUNT(*) INTO v_existe 
    FROM Prestamo 
    WHERE id_prestamo = p_id_prestamo AND id_usuario = p_id_usuario AND fecha_devolucion IS NULL;
    
    IF v_existe = 0 THEN
        SELECT 'Error: El préstamo no existe, no pertenece al usuario o ya fue devuelto.' AS MensajeError;
    ELSE
        START TRANSACTION;
        UPDATE Prestamo SET fecha_devolucion = CURRENT_DATE() WHERE id_prestamo = p_id_prestamo AND id_usuario = p_id_usuario;
        UPDATE Usuarios SET saldo_penalizacion = 0.00 WHERE id_usuario = p_id_usuario;
        COMMIT;
        SELECT 'Devolución y condonación realizadas con éxito.' AS MensajeExito;
    END IF;
END $$
DELIMITER ;

DELIMITER $$
CREATE OR REPLACE PROCEDURE RenovarPrestamo(IN p_id_prestamo INT, IN p_dias_extra INT)
BEGIN
    DECLARE v_existe INT DEFAULT 0;
    ...
    IF v_existe = 0 THEN
        SELECT 'Error: No se puede renovar un préstamo que no existe o ya está cerrado.' AS MensajeError;
    ELSEIF p_dias_extra <= 0 THEN
        SELECT 'Error: Los días extra de renovación deben ser mayores a 0.' AS MensajeError;
    ELSE
        START TRANSACTION;
        UPDATE Prestamo SET Num_Renovaciones = Num_Renovaciones + 1 WHERE id_prestamo = p_id_prestamo;
        UPDATE Prestamo SET fecha_prestamo = DATE_ADD(fecha_prestamo, INTERVAL p_dias_extra DAY) WHERE id_prestamo = p_id_prestamo;
        COMMIT;
        SELECT 'Renovación aplicada correctamente.' AS MensajeExito;
    END IF;
END $$
DELIMITER ;
...
-- 3.5. Cursores
CREATE TABLE IF NOT EXISTS Usuarios_Inactivos LIKE Usuarios;

DELIMITER $$
CREATE OR REPLACE PROCEDURE ac1111usuariosSinPrestamos()
BEGIN
    DECLARE usr ROW TYPE OF Usuarios;
    DECLARE fin INT DEFAULT 0;
    DECLARE cur_inactivos CURSOR FOR SELECT * FROM Usuarios WHERE Num_Prestamos = 0;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET fin = 1;
    
    OPEN cur_inactivos;
    bucle: LOOP
        FETCH cur_inactivos INTO usr;
        IF fin = 1 THEN LEAVE bucle; END IF;
        INSERT INTO Usuarios_Inactivos(id_usuario, nombre, email, fecha_registro, Num_Prestamos, saldo_penalizacion)
        VALUES (usr.id_usuario, usr.nombre, usr.email, usr.fecha_registro, usr.Num_Prestamos, usr.saldo_penalizacion);
    END LOOP;
    CLOSE cur_inactivos;
END $$
DELIMITER ;
...
DELIMITER $$
CREATE OR REPLACE PROCEDURE ac1111usuariosFiltroPrestamos(IN p_cantidad INT)
BEGIN
    DECLARE usr ROW TYPE OF Usuarios;
    DECLARE fin INT DEFAULT 0;
    DECLARE cur_filtro CURSOR FOR SELECT * FROM Usuarios WHERE Num_Prestamos = p_cantidad;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET fin = 1;
    
    OPEN cur_filtro;
    bucle: LOOP
        FETCH cur_filtro INTO usr;
        IF fin = 1 THEN LEAVE bucle; END IF;
        INSERT INTO Usuarios_Inactivos(id_usuario, nombre, email, fecha_registro, Num_Prestamos, saldo_penalizacion)
        VALUES (usr.id_usuario, usr.nombre, usr.email, usr.fecha_registro, usr.Num_Prestamos, usr.saldo_penalizacion);
    END LOOP;
...
-- 3.6. Funciones y Auxiliares
DELIMITER $$
CREATE FUNCTION obtenerAutorPorISBN(p_isbn VARCHAR(20))
RETURNS VARCHAR(100)
BEGIN
    DECLARE v_nombre_autor VARCHAR(100);
    SELECT A.nombre_autor INTO v_nombre_autor
    FROM Autor A
    INNER JOIN es_escrito E ON A.id_autor = E.id_autor
    WHERE E.isbn = p_isbn
    LIMIT 1;
    RETURN v_nombre_autor;
END $$
DELIMITER ;

DELIMITER $$
CREATE OR REPLACE PROCEDURE totalPrestamosLibro(IN p_isbn VARCHAR(20), OUT p_Total INT)
BEGIN
    SELECT COUNT(P.id_prestamo) INTO p_Total
    FROM Prestamo P
    INNER JOIN Ejemplares E ON P.id_ejemplar = E.id_ejemplar
    WHERE E.isbn = p_isbn;
END $$
...
-- Función con tu corrección exacta
DELIMITER $$
CREATE OR REPLACE FUNCTION categoriaPopularidadLibro(p_isbn VARCHAR(20))
RETURNS VARCHAR(15)
BEGIN
    DECLARE total_prestamos INT;
    DECLARE categoria VARCHAR(15);
    
    -- AQUÍ ESTABA EL ERROR: ya está corregido para que llame a 'totalPrestamosLibro' sin el ac12
    CALL totalPrestamosLibro(p_isbn, total_prestamos);
    
    IF total_prestamos IS NULL OR total_prestamos = 0 THEN 
        SET categoria = 'Nula';
    ELSEIF total_prestamos = 1 THEN 
        SET categoria = 'Baja';
    ELSEIF total_prestamos = 2 THEN 
        SET categoria = 'Media';
    ELSE 
        SET categoria = 'Alta';
    END IF;
...
-- 3.7. Script de Dashboard (Estadísticas)
CREATE TABLE IF NOT EXISTS informe_popularidad_libros(
    isbn VARCHAR(20) PRIMARY KEY,
    titulo VARCHAR(200),
    TotalEjemplares INT,
    TotalPrestamos INT,
    NivelPopularidad VARCHAR(15)
);

DELIMITER $$
CREATE OR REPLACE PROCEDURE actualizaDashboardLibro(IN p_isbn VARCHAR(20))
BEGIN
    DECLARE v_totalEjemplares INT;
    DECLARE v_totalPrestamos INT;
    DECLARE v_popularidad VARCHAR(15);
    
    SELECT COUNT(*) INTO v_totalEjemplares FROM Ejemplares WHERE isbn = p_isbn;
    
    CALL totalPrestamosLibro(p_isbn, v_totalPrestamos);
    SET v_popularidad = categoriaPopularidadLibro(p_isbn);
    
    INSERT INTO informe_popularidad_libros (isbn, titulo, TotalEjemplares, TotalPrestamos, NivelPopularidad)
    VALUES (p_isbn, (SELECT titulo FROM Libros WHERE isbn = p_isbn), v_totalEjemplares, v_totalPrestamos, v_popularidad)
    ON DUPLICATE KEY UPDATE 
        TotalEjemplares = v_totalEjemplares,
        TotalPrestamos = v_totalPrestamos,
        NivelPopularidad = v_popularidad;
END $$
DELIMITER ;

-- Ejecución de prueba del Dashboard
CALL actualizaDashboardLibro('978-0307474728');

-- Consulta final
SELECT * FROM informe_popularidad_libros;